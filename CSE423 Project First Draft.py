...
def draw_track():
    ...
    for i in range(len(track_segments)):
        ...
        # Draw track segment as a quad strip
...
def draw_car_model():
    ...
    def draw_wheel(x, y, z):
        ...
        gluDisk(gluNewQuadric(), 2, 5, 8, 1)  # Front disk
        glTranslatef(0, 0, 3)
        gluDisk(gluNewQuadric(), 2, 5, 8, 1)  # Back disk
        glColor3f(0.2, 0.2, 0.2)  # Reset to black
        glPopMatrix()
...
def draw_bike_model():
    ...
    def draw_wheel(y, z):
        glPushMatrix()
        glTranslatef(0, y, z)
        glRotatef(90, 1, 0, 0)  # Orient wheel properly
        glRotatef(wheel_rotation, 0, 0, 1)  # Rotate wheel based on vehicle movement
        gluCylinder(gluNewQuadric(), 8, 8, 2, 16, 1)  # Tire
        
        # Wheel rim and spokes
        glColor3f(0.7, 0.7, 0.7)  # Silver for rim
        gluDisk(gluNewQuadric(), 6, 8, 16, 1)  # Front disk
        glTranslatef(0, 0, 2)
        gluDisk(gluNewQuadric(), 6, 8, 16, 1)  # Back disk
        glColor3f(0.2, 0.2, 0.2)  # Reset to black
        glPopMatrix()
